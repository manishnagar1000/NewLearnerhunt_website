...
const SEOManage = () => {
  ...
  const handleCourseInputChange = (e, group, type = "text") => {
    ...
    if (group === "basic") {
      if (type == "select") {
        const selectedcourseObj = courseApi.find((obj) => obj._id == value.value);
  ...
  const handleSaveSEOData = () => {
    if (seoType == 1) {
      ...
      if (Object.values(newcourseErrors).some((error) => error)) {
        ...
        fd.append("twitter_description", twittercourseDetails.twitterdescription);
        ...
        fetch(process.env.NEXT_PUBLIC_API_ENDPOINT + "/admin/seomanager-course", {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("pt")}`,
          },
          method: "POST",
