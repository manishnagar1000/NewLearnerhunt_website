...
export default class AllCourses extends Component {
  ...
  getAssetList() {
    ...
    fetch(process.env.NEXT_PUBLIC_API_ENDPOINT + `/admin/course?id=-1`, {
      headers: {
            ...
            <table className={`table table-hover custom-table`}>
              <thead style={{ top: `8vh` }}>
                <tr>
                  ...
                  <th style={{ background: "var(--primary)" }}>Course Duration</th>
              ...
              <tbody>
                {this.state.courseList.map((clg, i) => {
                  return (
                      <tr key={i}>
                        <td style={{wordWrap:"break-word",whiteSpace:"unset"}}>{clg.course_name}</td>
                        <td style={{wordWrap:"break-word",whiteSpace:"unset"}}>{clg.course_duration}</td>
                        <td style={{wordWrap:"break-word",whiteSpace:"unset"}} onClick={(e)=>this.handleDelete(e,clg._id)}><DeleteForeverIcon/></td>
                        <td style={{wordWrap:"break-word",whiteSpace:"unset"}}><Link href={`addcourse?e=${clg._id}`}><EditIcon/></Link></td>
